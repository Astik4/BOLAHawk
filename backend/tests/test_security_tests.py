...
def test_jwt_flags_token_without_expiry():
    from app.scanner.security_tests.checks.jwt_flaws import JWTFlawsCheck
    import jwt as pyjwt
    token = pyjwt.encode({"user_id": 1, "username": "alice", "role": "user"}, key="secret", algorithm="HS256")
    findings = JWTFlawsCheck()._check_missing_expiry(token)
    assert len(findings) == 1
    assert "exp" in findings[0].evidence


def test_jwt_ignores_token_with_expiry():
    from app.scanner.security_tests.checks.jwt_flaws import JWTFlawsCheck
    import datetime, jwt as pyjwt
    exp = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=15)
    token = pyjwt.encode({"user_id": 1, "exp": exp}, key="secret", algorithm="HS256")
    assert JWTFlawsCheck()._check_missing_expiry(token) == []
